...
function InputComponent({ className, ...props }: React.ComponentProps<typeof Input>) {
  ...
  return <Input variant={variant} className={cn('rounded-l-none focus:z-1 flex-1', className)} {...props} />;
}
...
function CountrySelect({ disabled, value: selectedCountry, options: countryList, onChange }: CountrySelectProps) {
  ...
  // Ensure button height matches input height: map md->lg (h-10)
  const buttonSize = (variant === 'md' ? 'lg' : variant) as 'sm' | 'md' | 'lg';
        ...
        <ComboboxTrigger
          render={
            <Button
              ...
              className={cn(
                [
                  'flex gap-1 rounded-l-md rounded-r-none border-e-0 px-3 focus:z-10',
                  // Allow dial code text to fit without clipping
                  'min-w-16 w-auto',
