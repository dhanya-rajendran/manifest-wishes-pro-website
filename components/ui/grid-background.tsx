  ...
  const animatedBeams = React.useMemo(
    () =>
      Array.from({ length: Math.min(count, 12) }, (_, i) => {
        // Deterministic orientation and start position to avoid SSR/CSR mismatch
        const direction = i % 2 === 0 ? 'horizontal' : 'vertical';
        const startPosition = i % 3 === 0 ? 'start' : 'end';

        // Deterministic grid line selection within inner lines (exclude edges)
        const gridLine =
          direction === 'horizontal'
            ? ((i % (rows - 1)) + 1)
            : ((i % (cols - 1)) + 1);

        // Deterministic timing values (only affect animation, not initial DOM)
        const delay = (i % 5) * 0.25; // 0, 0.25, 0.5, 0.75, 1.0
        const duration = speed + ((i % 4) * 0.3); // slight variation per beam
